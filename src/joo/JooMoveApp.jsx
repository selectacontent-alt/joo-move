    ...
    <div className={`jm-wa-wrapper ${footerInView && !isOpen ? 'jm-hidden-float' : ''}`}>
      <div className={`jm-wa-window ${isOpen ? 'open' : ''}`}>
        ...
        <div className="jm-wa-footer">
          <a href={`https://wa.me/${cleanPhone(phone)}`} target="_blank" rel="noreferrer" onClick={() => setIsOpen(false)}>
...
function RequestMovePage({ services, navigate }) {
  ...
  return <main className="jm-wizard-page"><PageHero eyebrow={pick(language, 'طلب سريع بدون حساب', 'Quick request, no account')} title={pick(language, 'احكيلنا تفاصيل نقلتك', 'Tell us about your move')} description={pick(language, 'كل معلومة بتساعدنا نجهز الفريق والخامات والعربية المناسبة من أول مرة.', 'Each detail helps us assign the right crew, materials and truck from the start.')} />
    <section className="jm-container jm-wizard-shell"><div className="jm-wizard-progress">{WIZARD_STEPS.map(([ar, en], index) => <button key={ar} className={index === step ? 'active' : index < step ? 'done' : ''} onClick={() => index < step && setStep(index)}><span>{index < step ? <Check size={16} /> : index + 1}</span><b>{pick(language, ar, en)}</b></button>)}</div>
      <div className="jm-wizard-card"><div className="jm-wizard-title"><span>{pick(language, `الخطوة ${step + 1} من 6`, `Step ${step + 1} of 6`)}</span><h2>{pick(language, WIZARD_STEPS[step][0], WIZARD_STEPS[step][1])}</h2></div>
        <div className="jm-form-grid">
          ...
          {step === 1 && <><Field label={pick(language, 'محافظة النقل', 'Origin governorate')}><input value={form.origin_governorate} onChange={(e) => update('origin_governorate', e.target.value)} /></Field><Field label={pick(language, 'منطقة النقل *', 'Origin area *')}><input value={form.origin_area} onChange={(e) => update('origin_area', e.target.value)} /></Field><Field label={pick(language, 'العنوان الحالي بالتفصيل', 'Current address')} wide><textarea value={form.origin_address} onChange={(e) => update('origin_address', e.target.value)} /></Field><Field label={pick(language, 'محافظة الوجهة', 'Destination governorate')}><input value={form.destination_governorate} onChange={(e) => update('destination_governorate', e.target.value)} /></Field><Field label={pick(language, 'منطقة الوجهة *', 'Destination area *')}><input value={form.destination_area} onChange={(e) => update('destination_area', e.target.value)} /></Field><Field label={pick(language, 'عنوان الوجهة بالتفصيل', 'Destination address')} wide><textarea value={form.destination_address} onChange={(e) => update('destination_address', e.target.value)} /></Field></>}
...
function TrackPage() {
  ...
  return <main><PageHero eyebrow={pick(language, 'متابعة آمنة', 'Secure tracking')} title={pick(language, 'اعرف نقلتك وصلت لفين', 'See where your move stands')} description={pick(language, 'اكتب رقم الطلب ورقم الهاتف المستخدم وقت الحجز.', 'Enter the request number and phone used when booking.')} />
    <section className="jm-section"><div className="jm-container jm-track-layout"><form className="jm-track-form" onSubmit={track}><h2>{pick(language, 'بيانات الطلب', 'Request details')}</h2><Field label={pick(language, 'رقم الطلب', 'Request number')}><input value={input.requestNumber} onChange={(e) => setInput({ ...input, requestNumber: e.target.value.toUpperCase() })} placeholder="JM-2026-000123" /></Field><Field label={pick(language, 'رقم الهاتف', 'Phone number')}><input inputMode="tel" value={input.phone} onChange={(e) => setInput({ ...input, phone: e.target.value })} /></Field>{error && <div className="jm-form-error">{error}</div>}<button className="jm-btn jm-btn-red" disabled={loading}>{loading ? copy[language].common.loading : copy[language].nav.track}<Search /></button><p><ShieldCheck />{pick(language, 'لن تظهر أي بيانات بدون تطابق الرقمين.', 'No data is shown unless both fields match.')}</p></form>
...
function MyRequestsPage({ navigate }) {
  ...
  return <main><PageHero eyebrow={pick(language, 'حساب اختياري', 'Optional account')} title={pick(language, 'طلباتك المحفوظة', 'Your saved requests')} description={pick(language, 'الطلبات المرسلة من هذا الجهاز تظهر هنا. التتبع الآمن يحتاج رقم الطلب والهاتف.', 'Requests sent from this device appear here. Secure tracking still requires request number and phone.')} /><section className="jm-section"><div className="jm-container jm-saved-list">{saved.length ? saved.map((item) => <article key={item.requestNumber}><div><span>{pick(language, 'طلب نقل', 'Move request')}</span><h3>{item.requestNumber}</h3><small>{new Date(item.createdAt).toLocaleDateString(language === 'ar' ? 'ar-EG' : 'en-GB')}</small></div><button className="jm-btn jm-btn-soft" onClick={() => navigate('/track-request')}>{copy[language].nav.track}<ArrowLeft /></button></article>) : <div className="jm-empty-state"><Boxes /><h3>{pick(language, 'مفيش طلبات محفوظة على الجهاز ده', 'No requests saved on this device')}</h3><p>{pick(language, 'ابدأ طلب جديد أو تابع طلب سابق باستخدام الرقم والهاتف.', 'Start a new request or track an existing one using its number and phone.')}</p><button className="jm-btn jm-btn-red" onClick={() => navigate('/request-move')}>{copy[language].nav.request}</button></div>}</div></section></main>;
}
...
function MobileDock({ navigate, settings }) {
  ...
  return <div className="jm-mobile-dock"><a href={`tel:${cleanPhone(phone)}`}><Phone /><span>{pick(language, 'اتصال', 'Call')}</span></a><a href={`https://wa.me/${cleanPhone(phone)}`} target="_blank" rel="noreferrer" className="jm-whatsapp-color"><WhatsAppIcon /><span>{pick(language, 'واتساب', 'WhatsApp')}</span></a><button onClick={() => navigate('/request-move')}><Truck /><span>{copy[language].nav.request}</span></button></div>;
}
